...
function lisaaTili($formdata, $baseurl='') {
  ...
  // Tarkistetaan onko kaupunki määritelty ja se täyttää mallin.
  if (!isset($formdata['kaupunki']) || !$formdata['kaupunki']) {
    $error['kaupunki'] = "Kaupunki?";
  } else {
    if (!preg_match("/^[- '\p{L}]+$/u", $formdata["kaupunki"])) {
      $error['kaupunki'] = "Syötä kaupungin nimi ilman erikoismerkkejä.";
    }
  }
/*
...
function lahetaVaihtoavain($email,$url) {
  ...
  return mail($email,'Liikuntaryhmä-tilin salasanan vaihtaminen',$message);
}
